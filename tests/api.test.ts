...
import { castToApiSignature } from "../src/api/http/utils";
...
describe("sign check", () => {
  ...
  it("should not fail on sign check place order", async () => {
    ...
    let order: SDK.Order = {
      maker: test_acc.account_address,
      price: 1000_00000000000000000,
      qty: {
        base_qty: 100000000000000000,
        quote_qty: 0,
        base_asset: 1000000000000000000,
      },
      flags: {
...
describe("websockets", () => {
  it("connect and data consume example", async () => {
    ...
    async function handler(evt: any) {
      console.log(`Consume received ${evt}`);
    }
